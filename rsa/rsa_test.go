...
func TestRsaSign8Base64(t *testing.T) {
	...
	base64Sign1024, err := RsaSignPkcs8Sha1Base64([]byte(msg), base64Key1024.PrivateKey)
	assert.Nil(t, err)
	_, err = RsaSignPkcs8Sha1Base64([]byte(msg), hexPriKey2048)
	assert.NotNil(t, err)
	res := RsaVerifySignPkcs8Sha1Base64([]byte(msg), base64Sign1024, base64Key1024.PublicKey)
	assert.Equal(t, res, true)
	res = RsaVerifySignPkcs8Sha1Base64([]byte(msg), base64Sign1024, base64Key2048.PublicKey)
	assert.Equal(t, res, false)
	res = RsaVerifySignPkcs8Sha1Base64([]byte(msg), "11111", "badpubkey")
	assert.Equal(t, res, false)
	res = RsaVerifySignPkcs8Sha1Base64([]byte(msg), "11111", base64Key1024.PublicKey)
	assert.Equal(t, res, false)
	res = RsaVerifySignPkcs8Sha1Base64([]byte(msg), base64Sign1024, base64Key2048.PublicKey)
	assert.Equal(t, res, false)

	base64Sign2048, err := RsaSignPkcs8Sha1Base64([]byte(msg), base64Key2048.PrivateKey)
	assert.Nil(t, err)
	res = RsaVerifySignPkcs8Sha1Base64([]byte(msg), base64Sign2048, base64Key2048.PublicKey)
	assert.Equal(t, res, true)
	res = RsaVerifySignPkcs8Sha1Base64([]byte(msg), base64Sign2048, base64Key1024.PublicKey)
	assert.Equal(t, res, false)
	res = RsaVerifySignPkcs8Sha1Base64([]byte(msg), "11111", "badpubkey")
	assert.Equal(t, res, false)
	res = RsaVerifySignPkcs8Sha1Base64([]byte(msg), base64Sign2048, base64Key1024.PublicKey)
	assert.Equal(t, res, false)
	res = RsaVerifySignPkcs8Sha1Base64([]byte(msg), base64Sign2048, base64Key1024.PrivateKey)
	assert.Equal(t, res, false)
	res = RsaVerifySignPkcs8Sha1Base64([]byte(msg), base64Sign2048, base64Key2048.PrivateKey)
	assert.Equal(t, res, false)
}
